...
def propagate_amp(
    ...
    sim = model.simulate(length, n_records=n_records, plot=plot)
    return model, sim
...
def amplify(
    ...
    if p_bck is None:
        if Pp_bck == 0:
            model_fwd, sim_fwd = propagate_amp(
                p_fwd, eydf, length, Pp_fwd, n_records=n_records, plot=plot
            )
    ...
    while not done:
        ...
        sim_fwd = model_fwd.simulate(length, n_records=n_records, plot=plot)
        ...
        if bck_seeded:
            sim_bck = model_bck.simulate(length, n_records=n_records, plot=plot)
